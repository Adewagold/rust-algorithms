// This is a basic practice for command line arguments
use std::env;
use std::fs::File;
use image::png::PNGEncoder;
use image::ColorType;

use std::str::FromStr;

use num::Complex;
use num::complex::ComplexFloat;

pub fn parse_pair<T: FromStr>(s: &str, seperator: char) -> Option<(T,T)> {
    let name = "Adewale Adeleye Ayokanmi";
    let space_position = name.find(' ');
    print!("Space position is {:?}", space_position);
    print!("Next Space position is {:?}", space_position.iter().next());
    println!("First name {}", &name[..space_position.unwrap()]);
    println!("Last name {}", &name[space_position.unwrap() + 1..]);

    let found = s.find(',');
    print!("FOund {:?}", found);
    match s.find(seperator){
        None => None,
        Some(index) => {
            match (T::from_str(&s[..index]), T::from_str(&s[index+1..])){
                (Ok(l), Ok(r)) => Some((l,r)),
                _ => None
            }
        }
    }
}

/// parse a pair of floating-point numbers separated by a comma as a complex number
/// 
pub fn parse_complex(s: &str) -> Option<Complex<f64>>{
    match parse_pair(s,',') {
        Some((re,im)) => Some(Complex{re,im}),
        _ => None
    }
}

pub fn pixel_to_point(bounds: (usize, usize), 
                        pixel: (usize, usize),
                        upper_left: Complex<f64>, 
                        lower_right: Complex<f64>) -> Complex<f64> {
    let (width, height) = (lower_right.re - upper_left.re, upper_left.im - lower_right.im);
    Complex { 
        re: upper_left.re + pixel.0 as f64 * width / bounds.0 as f64, 
        im: upper_left.im - pixel.1 as f64 * height / bounds.1 as f64 
        // Why subtraction here? pixel.1 increases as we go down but the imaginary component increases as we go up.
    }
}

pub fn escape_time(c: Complex<f64>, limit: usize) -> Option<usize> {
    let mut z = Complex {re: 0.0, im: 0.0};
    for i in 0..limit{
        if z.norm_sqr() > 4.0 {
            return Some(i);
        }
        z = z * z + c;
    }
    None
}

pub fn render(pixels: &mut [u8],bounds: (usize,usize),upper_left: Complex<f64>, lower_right: Complex<f64>){
    assert!(pixels.len() == bounds.0 * bounds.1);
    for row in 0..bounds.1 {
        for column in 0..bounds.0 {
            let point = pixel_to_point(bounds, (column,row), upper_left, lower_right);
            pixels[row * bounds.0 + column] = match  escape_time(point, 255) {
                None => 0,
                Some(count) => 255 - count as u8
            };
        }
    }
}

pub fn write_image(filename:&str, pixels: &[u8], bounds: (usize, usize)) -> Result<(), std::io::Error>{
    let output = File::create(filename)?;
    let encoder = PNGEncoder::new(output);
    encoder.encode(pixels,bounds.0 as u32,bounds.1 as u32,ColorType::Gray(8))?;
    Ok(())
}

#[test]
fn test_pixel_to_point(){
    assert_eq!(pixel_to_point((100,200), (25,175), Complex { re: -1.0, im: 1.0 }, Complex { re: 1.0, im: -1.0 }),Complex{re:-0.5, im: -0.75});
}

#[test]
pub fn test_parse_complex(){
    assert_eq!(parse_complex("30,29"),Some(Complex { re: 30.0, im: 29.0 }));
}

#[test]
pub fn test_parse_pair(){
    assert_eq!(parse_pair::<i32>("",','),None);
    assert_eq!(parse_pair::<i32>("100,200",','),Some((100,200)));
}